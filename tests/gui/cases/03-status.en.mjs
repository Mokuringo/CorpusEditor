/**
 * Record status — English (en) variant.
...
export default {
  ...
  async run(ctx) {
    ...
    await check('「All」页签呈现未修改 / 待确认 / 已确认三态', async () => {
    ...
    await check('已确认：字段锁死 + 锁定提示 + 只能「Revert to edit」', async () => {
      ...
      assert(/locked/.test(state.noteText), `提示文案不对：${state.noteText}`)
      assert(state.lockedFields > 0, '已确认记录没有一个字段标成锁定')
      assertEqual(state.editable, 0, `已确认记录仍有 ${state.editable} 个可编辑控件`)
      assert(
        state.buttons.some((b) => /Revert to edit/.test(b)),
        `已确认状态缺「Revert to edit」：${JSON.stringify(state.buttons)}`
      )
      assert(
        !state.buttons.some((b) => /^Confirm & next$/.test(b)),
        `已确认状态不该再有「Confirm & next」：${JSON.stringify(state.buttons)}`
      )
      await shot('03-confirmed-locked-en')
    })

    await check('退回修改后字段解锁，可以再改', async () => {
      await ctx.clickText('Revert to edit')
      await ctx.sleep(600)
      const state = await evalJs(
        `return {
      ...
      const state = await evalJs(
        `const head = document.querySelector('.record-head')
         return {
           ...
           hasRevert: [...document.querySelectorAll('button')].some(b => /Revert/.test(b.textContent))
         }`
      )
      assert(state.modifiedFields > 0, '待确认记录没有一个字段标成已修改')
      assertEqual(state.lockedFields, 0, '待确认记录不该有锁定的字段')
      assert(/changes/.test(state.headText), `没显示改动数：${state.headText}`)
      assert(state.hasRevert, '待确认记录缺「Revert」入口')
      await shot('03-pending-en')
    })

    await check('「Deleted」页签里能找到删除项，编辑器给「Restore」', async () => {
      ...
      const state = await evalJs(
        `return {
           hasRestore: [...document.querySelectorAll('button')].some(b => b.textContent.trim() === 'Restore'),
           hasDelete: [...document.querySelectorAll('button')].some(b => /Delete this record/.test(b.textContent))
         }`
      )
      assert(state.hasRestore, '已删除记录应给「Restore」按钮')
      assert(!state.hasDelete, '已删除记录不该再显示「Delete this record」')
    })

    await check('新建记录带「New」标签，且没改动时不显示还原入口', async () => {
      ...
      const head = await evalJs(
        `const h = document.querySelector('.record-head')
         return {
           ...
           hasRevert: [...document.querySelectorAll('button')].some(b => /Revert whole record/.test(b.textContent))
         }`
      )
      assert(head.hasNewBadge, `编辑器里没有「New」徽章（列表项：${state.text}）`)
      // 「Revert whole record」只在 changeCount > 0 时渲染，新建且没改过 → 不出现，这是对的
      assert(!head.hasRevert, '新建且未改动的记录不该显示「Revert whole record」')
      await shot('03-new-record-en')
    })

    await check('给新建记录改动一下，「Revert whole record」出现且被禁用并说明原因', async () => {
      // 这条守住需求里明确的那句：「如果是新建的记录，还原整条就禁用」
      await ctx.typeInto('.field textarea', '（已编辑）把下面这句话翻译成英文')
      await ctx.sleep(700)

      const state = await evalJs(
        `const btn = [...document.querySelectorAll('button')].find(b => /Revert whole record/.test(b.textContent))
         if (!btn) return null
         return {
      ...
      assert(state, '新建记录改动后应出现「Revert whole record」按钮')
      assertEqual(state.headText.includes('New'), true, `改动后仍应带「New」标记：${state.headText}`)
      assert(state.disabled, '新建记录的「Revert whole record」必须禁用（没有原始值可还原）')
      assert(state.title.length > 0, '禁用按钮必须在 title 里说明为什么禁用')
      await shot('03-new-record-edited-en')
    })
  }
}
