/**
 * 核心操作页 —— 验证需求 ⑥（保持原样）、⑨（顺序流水线）、经线刻度与页签筛选。
 *
 * 进核心页走的是首页「继续上次」的「继续」按钮：它直接调 openFile(session.sourcePath)，
 * 不经过系统对话框，所以能在无人值守的测试里稳定复现。
 */
...
export default {
  ...
  async run(ctx) {
    ...
    await check('点「继续」能进核心页（走 openFile 而非对话框）', async () => {
    ...
    await check('标题栏标出格式与「只读」', async () => {
      ...
      assert(badges.some((b) => /只读/.test(b)), `标题栏没有只读徽章：${JSON.stringify(badges)}`)
    })
    ...
    await check('切到「已确认」页签只留 5 条', async () => {
      await switchTab(ctx, '已确认', { expectCount: 5 })
    ...
    await check('流水线条常驻在编辑区底部，显示位置与队列长度', async () => {
      ...
      assert(/确认并下一条/.test(bar.text), `流水线条缺「确认并下一条」：${bar.text}`)
