/**
 * Multi-select mode — English (en) variant.
 * Guards "entering/exiting multi-select must not shift list layout".
 */
...
export default {
  ...
  async run(ctx) {
    ...
    await shot('09-multiselect-en')

    // 暗色下再留一张。几何与主题无关（所以不重复断言），
    // 但勾选框在暗色底上的可见性只能靠眼睛确认，截图是唯一的证据。
    await evalJs(`document.querySelector('button[aria-label="Dark"]')?.click(); return true`)
    await ctx.sleep(600)
    await shot('09-multiselect-dark-en')
    await evalJs(`document.querySelector('button[aria-label="Light"]')?.click(); return true`)
    await ctx.sleep(600)
