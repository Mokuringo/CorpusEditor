/**
 * Home page — English (en) variant.
 * Asserts on the rendered English UI text. The en round uses English seed data (see samples.mjs).
 * Screenshot names carry the `-en` suffix (plan §1.4 pitfall 3).
 */
...
export default {
  ...
  async run(ctx) {
    ...
    await check('主题切换是三档（Light / Dark / System）', async () => {
      const modes = await evalJs(
        `return [...document.querySelectorAll('.titlebar .segmented button')].map(b => b.getAttribute('aria-label'))`
      )
      assertEqual(modes.join(','), 'Light,Dark,System', `主题按钮不对：${JSON.stringify(modes)}`)
    })

    await check('H1 点明这是指令微调数据工具', async () => {
      const h1 = await evalJs(`return document.querySelector('.home__title')?.textContent?.trim() || ''`)
      assert(/instruction-tuning/.test(h1), `标题没提到「instruction-tuning」：${h1}`)
    })

    await check('H1 按语义断成两行：Create and edit / instruction-tuning data', async () => {
      ...
      assertEqual(lines[0], 'Create and edit', `第一行不对：${JSON.stringify(lines)}`)
      assertEqual(lines[1], 'instruction-tuning data', `第二行不对：${JSON.stringify(lines)}`)
      // 每行自己不能再折行，否则断点又失控了
    ...
    await check('副文是商务口吻，不再出现「读入」这类口语', async () => {
      ...
      for (const word of ['读入', '关掉再打开']) {
        assert(!lede.includes(word), `副文里还留着中文口语词「${word}」：${lede}`)
      }
      assert(lede.includes('Supports'), `副文应以「Supports」引出格式：${lede}`)
    })
    ...
    await check('六张能力卡齐全且动词开头', async () => {
      ...
      assertEqual(
        titles,
        'Open read-only,Edit record by record,Bulk replace,Add records,Resume progress,Safe export',
        `能力卡标题变了：${titles}`
      )
    })

    await check('两个主操作按钮都在，且「New dataset」在左、「Open data file」在右', async () => {
      const btns = await evalJs(
        `return [...document.querySelectorAll('.home__actions button')].map(b => b.textContent.trim())`
      )
      assert(btns.some((t) => /Open data file/.test(t)), `缺「Open data file」：${JSON.stringify(btns)}`)
      assert(btns.some((t) => /New dataset/.test(t)), `缺「New dataset」：${JSON.stringify(btns)}`)
      // 顺序是产品要求：次要动作在左、主动作在右
      assert(
        /New dataset/.test(btns[0] ?? ''),
        `第一个按钮应是「New dataset」：${JSON.stringify(btns)}`
      )
      assert(
        /Open data file/.test(btns[1] ?? ''),
        `第二个按钮应是「Open data file」：${JSON.stringify(btns)}`
      )
    })

    await check('字段模板按格式流派分组，且含两种 ShareGPT 写法', async () => {
      await ctx.clickText('New dataset')
      ...
      for (const label of ['General', 'Instruction fine-tuning · SFT', 'Multi-turn', 'Preference comparison · DPO', 'Q&A']) {
        assert(labels.includes(label), `模板下拉缺分组「${label}」：${JSON.stringify(labels)}`)
      }
      // 分组顺序要和数据格式流派一致，不是随便排的
      assertEqual(
        labels.indexOf('Multi-turn') < labels.indexOf('Preference comparison · DPO'),
        true,
        `分组顺序不对：${JSON.stringify(labels)}`
      )

      const chatGroup = groups.find((g) => g.label === 'Multi-turn')
      assert(
      ...
      await shot('01-home-template-groups-en')
    ...
    await check('会话卡片的统计数字与播种一致', async () => {
      ...
      assert(stats, '找不到 alpaca 的会话卡片（需要先展开「Show all」）')
      assertEqual(stats['Total'], '120', '总条数不对')
      assertEqual(stats['Confirmed'], '5', '已确认数不对（播种了 5 条）')
      assertEqual(stats['Modified'], '3', '已改数不对（播种改了 3 条）')
      assertEqual(stats['Added'], '1', '新建数不对（播种追加了 1 条）')
      assertEqual(stats['Deleted'], '1', '已删数不对（播种删了 1 条）')
    })

    // 这条专门守住之前修掉的 bug：「Show all」按钮没绑 onClick
    await check('超过 3 份会话时「Show all」能展开并收起', async () => {
      ...
      assertEqual(expanded.label, 'Collapse', '展开后按钮文字应变「Collapse」')

      await shot('01-home-expanded-en')

      await ctx.clickText('Collapse')
      await ctx.sleep(400)
